...
int Simulador (void){
	...
	if (clase==0){
			...
			if (obtcodigo(number,clase)==0){
				...
				temp = r[rds]+0xffff; // es igual que -1;
				r[rds]=temp;		
...
void ActualizaALU (unsigned short op1, unsigned short op2, unsigned short r16, unsigned int r32){
		...
		int resultado32; // en este caso seria temp (r32 en el prototipo)
		short int signo16,signoop1,signoop2;
		
		signo16=(r16>>15)&1;
		signoop1=(op1>>15)&1;
		signoop2=(op2>>15)&1;		// pues la verdad que seria una variable que estemos usando de 16 bits si no la encontramos hay crear una y darle un nombre
													// r16 en el prototipo

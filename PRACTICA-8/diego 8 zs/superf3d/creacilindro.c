...
{
	FILE *fc;
	double circs[NUMRAD][3],circi[NUMRAD][3];
	int i;
	double ang;
	
	fc=fopen("cilindro.txt","wt");
	for (i=0;i<NUMRAD;i++)
	{
		...
		circi[i][0]=RAD*cos(ang);
		circi[i][2]=RAD*sin(ang);
		circi[i][1]=-ALTURA/2.0;
	}
	...
	for (i=0;i<NUMRAD;i++)
	{
		fprintf (fc,"%lf %lf %lf\n", circi[i][0], circi[i][1], circi[i][2]);
		fprintf (fc,"%lf %lf %lf\n", circi[(i+1)%NUMRAD][0], circi[(i+1)%NUMRAD][1], circi[(i+1)%NUMRAD][2]);
		fprintf (fc, "0 %lf 0\n", -ALTURA/2.0);
	}
